...
let title = ref<string>("Research two cities") // H1 tag value
let isLoading = ref<boolean>(false) // Loading bool
let search = ref<boolean>(true) // Search bool
let searched = ref<boolean>(false) // Search done bool
let error = ref<string>("") // Error message value
let isError = ref<boolean>(false) // If there is an error
let data_weather = ref(new Array<unknown>()) // Weather data Array
...
async function submitForm(query: string) {
    ...
    // Catches Axios errors (like a blank query)
    .catch(err => {
        isError.value = true
        isLoading.value = false
